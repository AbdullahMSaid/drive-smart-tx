...
export function LeadForm({
  ...
  const isPremium = (selectedVehicle?.category ?? data.vehicleCategory) === "premium";
  ...
  function validateStep(s: number): string | null {
    ...
    if (s === 1) {
      ...
      if (premiumDurationInvalid)
        return `Premium SUV rentals require a minimum of ${PREMIUM_SUV_MIN_DAYS} days.`;
      if (data.vehicleId) {
    ...
    if (s === 2) {
      ...
      if (ageNum < MIN_RENTAL_AGE_PLACEHOLDER)
  ...
  async function submit() {
    ...
    try {
      ...
      // Show success as soon as the raw request is safely stored. Email and AI
      // processing continue without making the visitor wait on the model.
      setResult(initial);

      void sendLeadTestEmail({
        data: {
        ...
        .catch((emailError) => console.warn("[lead email] failed:", emailError));

      void runAiLeadQualification(initial)
        .then((enriched) => setResult(enriched))
        .catch((pipelineError) =>
          console.warn("[lead qualification pipeline] continues in owner review:", pipelineError),
        );
    } catch (e) {
                ...
                <div className="mt-8">
                  ...
                  {step === 1 && (
                    <RentalStep
                      ...
                      duration={duration}
                      isPremium={isPremium}
                      premiumDurationInvalid={premiumDurationInvalid}
    ...
    <div className="rounded-2xl border border-gold/25 bg-gradient-to-br from-card via-card to-background p-7 relative overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="mt-6 space-y-3">
          <Benefit
            icon={ClipboardCheck}
            title="Checks basic rental fit"
            body="Age, license, insurance, and deposit readiness."
          />
          <Benefit
            icon={ListChecks}
            title="Identifies missing information"
            body="Flags what's still needed before pricing."
          />
          <Benefit
            icon={Sparkles}
            title="Helps the rental team respond faster"
            body="Summarizes your request in a clean, review-ready format."
          />
        </div>
        <div className="mt-6 rounded-lg border border-border bg-background/50 p-3 text-xs text-muted-foreground">
          Your answers are only used to review this rental request. Do not submit sensitive
          information such as Social Security numbers, full driver's-license numbers, or
    ...
    <div className="flex items-center gap-2">
      {STEPS.map((label, i) => (
        <div key={label} className="flex items-center gap-2 flex-1">
          ...
          {i < STEPS.length - 1 && (
            <div className={cn("h-px flex-1", i < step ? "bg-gold" : "bg-border")} />
          )}
        </div>
      ))}
    </div>
    ...
    <div className="space-y-5 rise-in">
      ...
      <Field label="Preferred contact method">
        <RadioGroup
          ...
          onValueChange={(v) => update("contactMethod", v as LeadFormData["contactMethod"])}
...
function RentalStep({
  ...
  return (
    <div className="space-y-5 rise-in">
      ...
      <Field
        label={
          data.pickupPreference === "delivery"
            ? "Delivery address or preferred area"
      ...
      >
        <Input
          ...
          placeholder="City or neighborhood in the DFW area"
          maxLength={120}
        />
      </Field>
      ...
      <Field label="Additional rental notes (optional)">
        <Textarea
          ...
          placeholder="Anything else the rental team should know?"
          maxLength={800}
        />
      </Field>
    </div>
  ...
  return (
    <div className="space-y-5 rise-in">
      <StepHeader
        ...
        desc="A few quick questions to help the rental team review your request."
      />
      ...
      <YN
        q="Are you prepared to provide a valid driver's license and proof of insurance before final approval?"
      ...
      <Field label="How soon do you need the vehicle?">
        <Select
          value={data.urgency}
          onValueChange={(v) => update("urgency", v as LeadFormData["urgency"])}
        >
          <SelectTrigger>
...
function ReviewStep({
  ...
  const est =
    data.vehicleId && data.pickupDate && data.returnDate
  ...
  const rows: [string, string][] = [
    ...
    [
      "Vehicle",
      veh?.name || (data.vehicleCategory === "unsure" ? "Not sure yet" : data.vehicleCategory),
    ],
    [
      "Dates",
      `${data.pickupDate || "—"} ${data.pickupTime || ""} → ${data.returnDate || "—"} ${data.returnTime || ""}${duration ? ` (${duration} day${duration === 1 ? "" : "s"})` : ""}`,
    ],
    [
      "Pickup or delivery",
      data.pickupPreference === "delivery" ? "Delivery requested" : "Customer pickup",
    ],
    ...(est
      ? ([
          [
  ...
  return (
    <div className="space-y-5 rise-in">
      ...
      <p className="text-xs text-muted-foreground">
        Do not submit Social Security numbers, payment-card details, full driver's-license numbers,
        or other highly sensitive information through this form.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="rise-in">
      ...
      <p className="mt-4 text-muted-foreground">
        Your information has been organized for review. A member of the rental team will contact you
        regarding vehicle availability, pricing, eligibility, and next steps.
      </p>
      <div className="mt-4 flex items-start gap-3 rounded-lg border border-gold/25 bg-gold/10 p-3">
        <Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-gold" />
        <p className="text-sm text-muted-foreground">
          Our AI-assisted review is processing now. You can leave this page—an email update will be
          sent after the review is complete.
        </p>
      </div>
      ...
      <dl className="mt-6 grid gap-3 rounded-lg border border-border bg-secondary/40 p-4 text-sm sm:grid-cols-2">
        ...
        <Row
          k="Requested dates"
          v={`${result.data.pickupDate} → ${result.data.returnDate}${result.rentalDurationDays ? ` (${result.rentalDurationDays}d)` : ""}`}
        />
